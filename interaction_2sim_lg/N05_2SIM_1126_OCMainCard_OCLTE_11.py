...
@author: jingjing.nan
ScriptName:SLAB_N05_Interaction_2SIM_1126_OpenCloseMainCard_OpenCloseLTE_11
Function:  sim2 is main card and data card
            1. Close Maincard Sim2,then maincard change to Sim1
            2. OpenSim2,and Close LTE,and open LTE
'''
import android
...
def OCMC_OCL():
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        time.sleep(10)
        passFlag1 = CloseMainCard(1)
        time.sleep(40)
        passFlag2 = OpenMainCard(1)
        time.sleep(40)
        passFlag3 = CloseLte(0)
        time.sleep(30)
        passFlag4 = OpenLte(0)
        time.sleep(30)
        if passFlag1 & passFlag2 & passFlag3 & passFlag4:
            passTimes += 1
        else:
            failTimes += 1
