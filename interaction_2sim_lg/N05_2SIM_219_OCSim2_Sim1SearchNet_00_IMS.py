...
@author: songchao.chu
ScriptName:SLAB_N05_Interaction_2SIM_219_OpenCloseSim2_Sim1SearchNet_00_IMS.py
...
def OpenCloseSIM_SearchNet():
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        searchFlag = nwObject.searchingNet(droid, 0)
        time.sleep(20)
        openFlag = OpenSim(1)
        searchFlag = nwObject.searchingNet(droid, 0)
        time.sleep(10)
        droid.log("===CloseSim(1)=" + str(closeFlag) + "===OpenSim(1)=" + str(openFlag) + "===searchingNet(1)=" + str(searchFlag))
        if closeFlag & searchFlag & openFlag:
            passTimes += 1
        else:
            failTimes += 1
