...
@author: panbin.ai
CaseName: 
   SLAB_N05_Interaction_DS_1115_OpenCloseSim1_ExchangeMainCard_IMS_00
Fuction: 
    SIM1 MAIN CARD,SIM1 DATA CARD
    OpenCloseSim1_EXchangeMainCard
'''
import android
...
def CheckIMS(sim):
        ...
        if netFlag == "4G" and VolteFlag:
            ...
            return False
def OpenCloseSim_ExchangeMainCard(sim):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        closesim1flag = nwObject.closeSim(droid, sim)
        time.sleep(15)
        opensim1flag = nwObject.openSim(droid, sim)
        time.sleep(15)
        changemainflag = callObject.setMainCard(droid, sim)
        time.sleep(10)
        droid.log("closesim1flag===" + str(closesim1flag) + "  changemainflag===" + str(changemainflag))
        time.sleep(20)
        if  opensim1flag and closesim1flag and changemainflag:
            passTimes += 1
        else:
            failTimes += 1
...
if droid.sendCommand("IsPluginSimCard 0", "OK") and droid.sendCommand("IsPluginSimCard 1", "OK"):
    ...
    if SelLTE(sim) and CheckIMS(sim):
        OpenCloseSim_ExchangeMainCard(0)
    else:
