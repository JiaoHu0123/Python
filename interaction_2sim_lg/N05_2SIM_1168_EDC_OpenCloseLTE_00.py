...
@author: jingjing.nan
CaseName:SLAB_N05_Interaction_2SIM_1168_ExchangeDataCard_OpenCloseLTE_00
Function:
    sim1 main card, data card. sim1 exchangeDataCard and opencloseLTE
...
def MDC_OCL(droid, testTimes, hold_time):
    ...
    openlte=OpenLte(1)
    time.sleep(40)
    if DataCard and closelte & openlte:
        flag = True
    else:
        flag = False
    return flag
...
if droid.sendCommand("IsPluginSimCard 0", "OK") and droid.sendCommand("IsPluginSimCard 1", "OK"):
    ...
    droid.log("pluginSim two card")
    cardtype="DUAL"
    if callObject.setMainCard(droid, 0):
