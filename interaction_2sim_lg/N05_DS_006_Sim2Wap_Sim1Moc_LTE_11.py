...
@author: panbin.ai
CaseName: SLAB_N05_Interaction_DS_006_Sim1Wap_Sim2Moc_LTE_11
Fuction:sim2 maincard sim2 datacard sim2wap sim1moc LTE network
'''
import android
...
def wapMoc(call_number):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        droid.log("wapflag===" + str(wapflag) + "  mocflag===" + str(mocflag))
        time.sleep(5)
        callObject.endCall(droid)
        if  wapflag and mocflag:
            passTimes += 1
        else:
            failTimes += 1
...
if droid.sendCommand("IsPluginSimCard 0", "OK") and droid.sendCommand("IsPluginSimCard 1", "OK"):
    ...
    time.sleep(10)
    callnum = callObject.getCardType(droid, 0)
