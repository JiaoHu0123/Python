...
@author: songchao.chu
ScriptName:SLAB_N05_Interaction_2SIM_259_OpenCloseSim1_Sim2SelNet_00_LTE.py
Function:  MainCard:SIM1&DataCard:SIM1
           OpenCloseSim1 and Sim2SelNet(LTE)
'''
import android
...
def OpenCloseSIM_SelNet():
    ...
    for i in range(1,int(testTimes)+1):
        ...
        droid.log("==================step1:  close sim"+str(1)+",and wait 5s")
        time.sleep(5)
        passFlag1=CloseSim(0)
        nwObject.startSearchNet(droid, 1)
        passFlag3= nwObject.selGSM(droid, 1)
        time.sleep(20)
        passFlag2=OpenSim(0)
        passFlag4= nwObject.selGSM(droid, 1)
        time.sleep(10)
        droid.log("===CloseSim(1)="+str(passFlag1)+"===OpenSim(1)="+str(passFlag2)+"===seLNetSim(2)="+str(passFlag3))
