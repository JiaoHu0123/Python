...
@author: panbin.ai
CaseName: 
    SLAB_N05_Interaction_DS_927_EMC+ExchangeMainCard_IMS_00
Fuction: 
    SIM1 MAIN CARD,SIM1 DATA CARD
    EMC+ExchangeMainCard
'''
import android
...
def EMC_ChangeMainCard(sim):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        changeMainCardFlag = callObject.setMainCard(droid, sim)
        emcflag = callObject.emcCall(droid, emcNum, holdTime)
        droid.log("emcflag===" + str(emcflag) + "  changeMainCardFlag===" + str(changeMainCardFlag))
        time.sleep(5)
        callObject.endCall(droid)
        if  emcflag and changeMainCardFlag:
            passTimes += 1
        else:
            failTimes += 1
