...
@author: jingjing.nan
ScriptName:SLAB_N05_Interaction_2SIM_1119_OpenCloseViceCard_OpenClosedata_00
Function:  sim1 is main card and data card
            1. Close Vice Sim2,Open Vice card SIM2,the main card data card still Sim1
            2. Close Data ,Open Data,Set to default settings
'''
import android
...
def OCVC_OCD():
    ...
    for i in range(1,int(testTimes)+1):
        ...
        openData=OpenData(0)
        time.sleep(30)
        if closeVice & openVice & closeData & openData:
            passTimes+=1
        else:
            failTimes+=1
