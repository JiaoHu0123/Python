...
@author: jiao.hu
ScriptName:SLAB_N05_Interaction_2SIM_673_ExchangeDataCard_SIM1SearchNet_11_LTE.py
Function:
dual simCard:Sim2 main card,sim2 data card
1. datacard: Sim2->Sim1
2. Sim1 SearchNet
3. datacard: Sim1->Sim2
'''
import android
...
def ChangeDataCard_SearchNet():  
    ...
    for i in range(1,int(testTimes)+ 1):
        ...
        Flag1=callObject.setDataCard(droid, 0)   #Select datacard sim1
        time.sleep(10)
        Flag2=nwObject.searchingNet(droid, 0)  #Sim1 Search network
        droid.log("SIM1 Search network===pass")
        time.sleep(10)
        Flag3=nwObject.searchingNet(droid, 1)  #MainCard2 Search network
        time.sleep(10)
        Flag4=nwObject.selLTE(droid, 1)   #select MainCard2 to LTE Network
        sleep(5)
        if Flag1 and Flag2 and Flag3 and Flag4:
            passTimes+=1
        else:
            failTimes+=1
        callObject.setDataCard(droid, 1)   #Select datacard sim2
        time.sleep(30)
...
if droid.sendCommand("IsPluginSimCard 0", "OK") and droid.sendCommand("IsPluginSimCard 1", "OK"):
    ...
    time.sleep(30)
    callObject.setDataCard(droid, 1)   #set Sim2 main card,sim2 data card
    time.sleep(30)
