...
@author: songchao.chu
ScriptName:SLAB_N05_Interaction_2SIM_DS_227_OpenCloseSim_Sim1SearchNet_00_LTE.py
Function:  MainCard:SIM1&DataCard:SIM1
           OpenCloseSim2 and Sim1SearchNet(LTE)
'''
import android
...
def OpenCloseSIM_SearchNet():
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        time.sleep(5)
        passFlag1 = CloseSim(1)
        passFlag3 = nwObject.searchingNet(droid, 0)
        time.sleep(20)
        passFlag2 = OpenSim(1)
        passFlag4 = nwObject.searchingNet(droid, 0)
        time.sleep(10)
        droid.log("===CloseData(1)=" + str(passFlag1) + "===OpenData(1)=" + str(passFlag2) + "===searchingNet(1)=" + str(passFlag3))
