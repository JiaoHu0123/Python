...
@author: panbin.ai
CaseName: 
   SLAB_N05_Interaction_DS_1113_OpenCloseSim1_OpenCloseData_IMS_00
Fuction: 
    SIM1 MAIN CARD,SIM1 DATA CARD
    OpenCloseSim1_OpenCloseData
'''
import android
...
def CheckIMS(sim):
        ...
        if netFlag == "4G" and VolteFlag:
            ...
            return False
def OpenCloseSim_OpenCloseData(sim1, sim2):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        closeDataflag = nwObject.closeData(droid, sim2)
        time.sleep(15)
        openDataflag = nwObject.openData(droid, sim2)
        droid.log("closesim1flag===" + str(closesim1flag) + "  openDataflag===" + str(openDataflag))
        time.sleep(20)
        if  opensim1flag and closesim1flag and closeDataflag and openDataflag:
            passTimes += 1
        else:
            failTimes += 1
...
if droid.sendCommand("IsPluginSimCard 0", "OK") and droid.sendCommand("IsPluginSimCard 1", "OK"):
    ...
    if SelLTE(sim) and CheckIMS(sim):
        OpenCloseSim_OpenCloseData(0, 1)
    else:
