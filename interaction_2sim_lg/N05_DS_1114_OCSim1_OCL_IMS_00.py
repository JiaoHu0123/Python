...
@author: panbin.ai
CaseName: 
   SLAB_N05_Interaction_DS_1114_OpenCloseSim1_OpenCloseLTE_IMS_00
Fuction: 
    SIM1 MAIN CARD,SIM1 DATA CARD
    OpenCloseSim1_OpenCloseLTE
'''
import android
...
def CheckIMS(sim):
        ...
        if netFlag == "4G" and VolteFlag:
            ...
            return False
def OpenCloseSim_OpenCloseLTE(sim):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        time.sleep(15)
        openlteflag = nwObject.openLte(droid)
        droid.log("closesim1flag===" + str(closesim1flag) + "  openlteflag===" + str(openlteflag))
        time.sleep(20)
        if  opensim1flag and closesim1flag and openlteflag and closelteflag:
            passTimes += 1
        else:
            failTimes += 1
