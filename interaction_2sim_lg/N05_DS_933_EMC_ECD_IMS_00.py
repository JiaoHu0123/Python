...
@author: panbin.ai
CaseName: 
    SLAB_N05_Interaction_DS_933_EMC+ExchangeDataCard_IMS_00
Fuction: 
...
def EMC_ChangeDataCard(sim):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        droid.log("emcflag===" + str(emcflag) + "  changeDataCardFlag===" + str(changeDataCardFlag))
        time.sleep(5)
        callObject.endCall(droid)
        if  emcflag and changeDataCardFlag:
            passTimes += 1
        else:
            failTimes += 1
