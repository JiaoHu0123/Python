...
@author: panbin.ai
CaseName: 
    SLAB_N05_Interaction_DS_1111_OpenCloseSim2_Sim1MOC_IMS_11
Fuction: 
    SIM2 MAIN CARD,SIM2 DATA CARD
    OpenCloseSim2_Sim1MOC
'''
import android
...
def OpenCloseSim_Moc(callNumber, sim):
    ...
    for i in range(1, int(testTimes) + 1):
        ...
        closesimflag = nwObject.closeSim(droid, sim)
        time.sleep(5)
        mocflag = callObject.voiceCall(droid, callNumber, holdTime)
        callObject.endCall(droid)
        opensimflag = nwObject.openSim(droid, sim)
        droid.log("closesimflag===" + str(closesimflag) + "  mocflag===" + str(mocflag) + "  closesimflag===" + str(opensimflag))
        if  closesimflag and mocflag and opensimflag:
            passTimes += 1
        else:
            failTimes += 1
